	...
	*/
	function check_sms_content($data){
		...
		foreach ($arr_ques as $question) {
			if ($question['T31TemplateQuestion']['question_type'] == QUESTION_SMS) {
...
// 			$T20OutSchedule['status'] = STATUS_CALLING;
	...
	function status() {
		...
		if (!empty($data)) {
			...
			$question_types = array(QUESTION_BASIC, QUESTION_AUTH, QUESTION_COUNT, QUESTION_SMS, QUESTION_AUTH_CHAR); //20161129 Update by Linh : include sms question
