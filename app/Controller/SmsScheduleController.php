...
class SmsScheduleController extends AppController {
	...
	function check_same_schedule($data){
		$count_schedule_exist = $this->T200SmsSendSchedule->checkSameSchedule(
			$data['schedule_id'],
			$data['create_date'],
			$data['template_id'],
			$data['list_id'],
	...
	function save($type = null) {
		...
		if($type == "create" || $type == "duplicate"
			...
			$T200SmsSendSchedule["entry_user"] = $user_id;
			if ($type == "duplicate") {
				$T200SmsSendSchedule["entry_program"] =  $this->name.'_Duplicate_Schedule';
			}else {
				$T200SmsSendSchedule["entry_program"] =  $this->name.'_Create_Schedule';
			}
		}
